...
dni <- 0.08    # mortality of SHR(-) plant upon viral infection 
...
## visualization
if (tr%%50==0){
pname <- paste("plot_b",seed,"_",Rshrp*100,"_",lp*100,"_tr",tr,".png",sep="")
...
cvi <- rbinom(s*s,1,vi)*(1-vt)*(1-(pt-1)*(pt-2)/2) #0.1% of plants are randomly attacked by the virus
vt <- vt+cvi
## death
...
## plots and record
if (t%%50==0){
pname <- paste("plot_b",seed,"_",Rshrp*100,"_",lp*100,"_t",t,".png",sep="")
png(pname,width=800,height=800)
plot(0,0,xlim=c(0,s),ylim=c(0,s),type="n",xlab="",ylab="")
for (x in 1:s){
points(rep(x,s),1:s,col=rgb((1-pt[x,])^2,1-pt[x,]*(pt[x,]-1)/2,(1-pt[x,])^2),pch=19,cex=1) # SHR(-): green; SHR(+): magenta
...
fname <- paste("output_b",seed,"_",Rshrp*100,"_",lp*100,".csv",sep="")
...
pname <- paste("fig_b",k,"_",j,".png",sep="")
png(pname,width=800,height=800)
...
for (i in 1:10){ 
fname <- paste("output_b",i,"_",k,"_",j,".csv",sep="")
tt <- read.csv(fname)
par(new=T)
...
write.csv(tvr,file="final_occupancy_b.csv")
