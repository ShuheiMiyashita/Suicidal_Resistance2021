#Text S2
#An R script for simulation of suicidal population resistance of SHR(+) plants
...
## visualization
if (tr%%50==0){
pname <- paste("plot",seed,"_",Rshrp*100,"_",lp*100,"_tr",tr,".png",sep="")
...
## plots and record
if (t%%50==0){
pname <- paste("plot",seed,"_",Rshrp*100,"_",lp*100,"_t",t,".png",sep="")
png(pname,width=800,height=800)
plot(0,0,xlim=c(0,s),ylim=c(0,s),type="n",xlab="",ylab="")
for (x in 1:s){
points(rep(x,s),1:s,col=rgb((1-pt[x,])^2,1-pt[x,]*(pt[x,]-1)/2,(1-pt[x,])^2),pch=19,cex=1) # SHR(-): green; SHR(+): magenta
...
fname <- paste("output",seed,"_",Rshrp*100,"_",lp*100,".csv",sep="")
...
for (i in 1:10){ 
fname <- paste("output",i,"_",k,"_",j,".csv",sep="")
tt <- read.csv(fname)
par(new=T)
...
write.csv(tvr,file="final_occupancy.csv")
